...
export const DATA = {
  ...
  academicProjects: [
    ...
    },
    {
      ...
      dates: "2024",
      active: true,
      description: "Designing cooling channels for modern chips means choosing between slow, expensive simulation or unreliable rules of thumb. This project trained a machine-learning model on simulation results so engineers could predict how a new design would perform in seconds instead of days.",
      longDescription: "As electronics get smaller and more powerful, keeping them cool gets harder — and the shapes that cool them best are increasingly exotic. One promising approach uses channels covered in small dimples, which stir the coolant into carrying more heat away. The problem is that nobody has reliable formulas for these shapes: the only trustworthy way to evaluate a design is a detailed computer simulation, which can take days per candidate. Engineers can't explore many options on that schedule, so they end up over-designing or settling.\n\nThis project replaced most of that waiting with machine learning. We generated a training set of simulations covering a range of dimpled-channel designs, then taught a model to predict, for a geometry it had never seen, how well it would cool and how hard the pump would have to work.\n\nThe model's predictions landed close to the true simulated values — within roughly a fifth of the actual figure for both cooling performance and pumping cost. That's accurate enough to screen dozens of candidate designs in seconds and send only the best few to full simulation, turning a days-long exploration into an afternoon.\n\nRelevant publication:\n\n- Shaeri, M. R.; Randriambololona, A. M.; & Adhikari, D. \"Machine Learning Algorithm for Predicting Heat Transfer Coefficient and Pressure Drop in Dimpled Ducts.\" ASME Heat Transfer Summer Conference, 2024.\n\n## Technical detail\n\nThe model is a fully connected artificial neural network trained on a limited CFD-generated dataset of water-cooled dimpled ducts, with no pre-existing input-output correlation to lean on. On held-out dimpled-duct geometries it predicted heat transfer coefficients within ±17% and pressure drops within ±19% of the simulated ground truth — sufficient fidelity to screen candidate geometries before committing any of them to full CFD or experiment.",
  ...
  personalProjects: [
    ...
    },
    {
      ...
      active: true,
      description:
        "An iPhone app that answers \"what can I cook with what I have?\" Tell it what's in your kitchen, and an AI running on my own hardware writes a recipe around it. You can then adjust it by conversation — make it quicker, dairy-free, spicier — or browse a library of ready-made recipes.",
      longDescription:
        "Most recipe apps work backwards: you pick a recipe first, then go shopping for it. The question people actually face at 6pm is the opposite — here's what's in my kitchen, what can I make with it?\n\nThat's the core of this app, but it grew into a full kitchen companion:\n\n- **Generate from your pantry.** Type in what you have — plus anything else you care about, like \"quick dinner\" or \"no dairy\" — and the AI writes a complete recipe around it. Your ingredients guide the dish rather than restrict it, so the model can suggest a few additions if they'd make it better. A Surprise Me mode generates from pantry staples alone when you haven't typed anything.\n- **Pantry matching on every recipe.** The app keeps track of what's in your kitchen and highlights, ingredient by ingredient, what you already have and what's missing — on anything you're looking at, generated or not.\n- **Import anything.** Found a recipe on TikTok, YouTube, a random blog, or a handwritten card? Send the link or a photo of it and the app pulls it into your library. Receipt photos work too — snap the receipt and your pantry updates itself.\n- **Adapt by conversation.** Tell any recipe what to change — make it quicker, spicier, feed six, swap what you don't have — and it rewrites itself instead of making you start over. Works the same on imported recipes and library ones.\n- **Explore and collect.** A browse page with ready-made recipes you can filter by cuisine, diet, or style — plus natural-language search — and a personal library where everything you've saved or generated lives. Cook mode walks through the chosen recipe step by step.\n\nTwo design decisions make it practical at scale. The AI serving layer can run on hardware in my home instead of a paid API, so generation costs nothing and pantry data stays private. And the app remembers every recipe it has ever produced: when a similar combination is requested again — by you or anyone else — the answer comes back instantly, and popular combinations are pre-loaded before anyone asks. Most requests never touch the model at all.\n\n## Under the hood\n\nThe app is React Native (Expo, TypeScript). Generation runs on llama3.2:3b served by Ollama on my home machine, reachable from the phone over Tailscale; a separate alternative backend is a Flask service around a 560M-parameter BLOOM model, fine-tuned for recipes, at ~8s inference with 100% valid-JSON output.\n\nThe ingredient ordering that feeds the prompt isn't naive: ingredients are embedded with nomic-embed-text and ranked by semantic relevance, with section dampening and usage-frequency weighting so the items that should drive the dish lead the prompt.\n\nThe cache is three tiers — AsyncStorage on device (L1), a shared Supabase table (L2), then the model (L3). The cache key is a SHA256 of the normalized ingredient list (lowercased, deduplicated, sorted), with universal staples like salt and oil excluded, so superficially different requests hit the same entry. A background job pre-warms L2 from popular combinations pulled via the Spoonacular API, targeting a >85% hit rate; at 1,000 daily users that's the difference between ~$5/day and ~$0.50/day of inference.",
    ...
    },
    {
      ...
      longDescription:
        "Lots of great local businesses — high ratings, loyal customers — have a terrible website or none at all. And they know it, but every pitch they've ever heard is \"pay me and I'll build you one.\" It's an easy pitch to ignore.\n\nThis pipeline flips the order: build first, talk second. Here's how a business goes from \"invisible online\" to \"looking at their new site\":\n\n- **Learn the industry once, reuse it forever.** Before any business is contacted, I study the best websites in that industry — what sections they have, what makes them feel trustworthy — and distill it into a blueprint. Every site in that industry afterward starts from that blueprint instead of a blank page.\n- **Find the right businesses.** The filter is deliberately specific: well-reviewed businesses (high ratings, plenty of reviews — proof they're good at what they do) with no website or a clearly broken one. Roughly half of any list gets cut after a human double-checks it.\n- **Build the demo from their own material.** Their photos, their services, their customers' actual reviews — pulled from what's already public — assembled into a complete, mobile-friendly site about *their* business. Nothing on the page is made up.\n- **Quality-check before anyone sees it.** Every demo is gone over page by page: phone numbers and buttons work, reviews are real and credited, it looks right on a phone screen, and it loads fast.\n\nThe result changes the conversation entirely. Instead of \"would you like a website?\", it's \"here's your website — want it?\" Nothing is ever sent to a business without my explicit sign-off, and if the owner says yes, the same process carries through to the real launch: hosting, their domain, contact forms, and a short lesson in how to run it.\n\n## Under the hood\n\nThe pipeline is five phases, each with a written output so it's repeatable across industries:\n\n- **Vertical research (once per industry).** Audit 5–8 strong competitors — page structure, hero patterns, CTAs, trust signals, mobile UX — and distill it into a template spec: the canonical sections and design tokens every site in that vertical needs.\n- **Prospect discovery.** Google Maps search filtered to businesses with 4.5+ stars, 20+ reviews, and no site or a broken one. About half of any raw list fails manual verification, so every prospect carries a Maps link for checking.\n- **Content gathering.** Everything public: the Maps listing, every Google review with attribution, photos, posts, existing site content, social media. For redesigns, the existing site is crawled page by page and their real photos and bios are reused — the before/after is the pitch.\n- **Build.** The template spec plus the business's real material goes into a self-contained, responsive HTML demo through an LLM-assisted editor. Nothing on the page is invented.\n- **Quality gate.** Checked against the spec before anyone sees it: all sections present, click-to-call correct, reviews real and attributed, responsive at 375/768/1440px, loads under 3 seconds, zero placeholder content.\n\nOutreach is a hard gate — nothing is sent unless I explicitly decide. Demos are deployed to Vercel or GitHub Pages only when ready to share; internal previews run over a private Tailscale-served port.",
