...
function cadGallery() {
  return `<section id="cad" class="cad" aria-labelledby="cad-heading"><h4 id="cad-heading">Guitar models</h4><label for="cad-model">Model</label><div class="cad-controls"><select id="cad-model"><option value="gt-partscaster">GT partscaster</option><option value="les-paul">Les Paul · dual humbuckers</option></select><button type="button" id="cad-load">Load 3D model</button><button type="button" id="cad-reset" hidden>Reset view</button></div><p id="cad-status" role="status" aria-live="polite">Choose a model to load (internet required).</p><div id="cad-viewer" class="cad-viewer" hidden><div id="cad-render"></div><img id="cad-poster" src="${dataImage('/demos/gt-guitar/assets/poster.webp')}" data-gt="${dataImage('/demos/gt-guitar/assets/poster.webp')}" data-lp="${dataImage('/demos/agent-cad/assets/les-paul-poster.webp')}" alt="Guitar model preview"></div></section>`;
}
